...
import { staffTypes as department } from '$lib/server/db/schema/';
...
export const actions: Actions = {
	...
	edit: async ({ request, locals }) => {
		...
		try {
			...
			setError(form, 'name', 'Position name already exists.');
			return message(form, {
				type: 'error',
				text:
					err.code === 'ER_DUP_ENTRY'
						? 'Position name is already taken. Please choose another one.'
						: err.message
			});
		}
	}
