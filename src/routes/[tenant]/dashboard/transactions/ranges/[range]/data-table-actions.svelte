<script lang="ts">
	...
	let {
		...
		date,
		booker
	}: { id: string; booker: string; recieptLink: string; date: string } = $props();
</script>
...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		...
		<DropdownMenu.Item
			><a href="/dashboard/transactions/single/{id}" target="_blank" class={dropdownClass}
				><ExternalLink /> Check out Transaction Details</a
			></DropdownMenu.Item
		>
		<DropdownMenu.Item
			><a href="/dashboard/user/{booker}" class={dropdownClass}>
				<ExternalLink /> Check out Reciever's Profile</a
			></DropdownMenu.Item
		>
		<DropdownMenu.Item
